(function($){
    acf.fields.image_crop = acf.field.extend({
        ...
        remove: function(e) {
            the_img = e.$el.parent().parent().parent().find('#the_cropped_img').attr('src');
            var data = {
                action: 'acf_image_crop_delete_crops',
                renditions: e.$el.data('renditions'),
                img: the_img
            }
...
// changed END
    ...
    function performCrop($field){
        if(!$field.find('.crop-stage').hasClass('loading')){
            ...
            var renditions = $options.data('renditions');
            ...
            var data = {
                ...
                renditions: $options.data('renditions'),
                postid: $options.data('postid')
            }
            $.post(ajaxurl, data, function(data, textStatus, xhr) {
                if(data.success){
