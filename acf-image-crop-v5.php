    ...
    */
    ...
    function initialize() {
        ...
        */
        ...
        add_action( 'wp_ajax_acf_image_crop_delete_crops', array( &$this, 'delete_crops' ) );
...
// added END
        ...
        // renditions
        acf_render_field_setting( $field, array(
            'label'         => __('Custom renditions','acf-image_crop'),
            'instructions'  => __('Comma separated array of widths for each rendition. Leave blank for no renditions.','acf-image_crop'),
            'type'          => 'text',
            'name'          => 'renditions',
            'class'         => 'custom-target-renditions custom-target-renditions',
        ));
    ...
    function render_field( $field ) {
       ...
       // enqueue
        ...
        $div_atts = array(
            ...
            'data-renditions'       => $field['renditions'],
            'data-postid'           => $post->ID,
        );
        $input_atts = array(
            'type'                  => 'hidden',
...
<div <?php acf_esc_attr_e( $div_atts ); ?>>
    ...
    <div class="view show-if-value acf-soh">
      ...
      $response = array();
      $response[] = $url;
      $halfUrl = str_replace("@2x","",$url);
      $response[] = $halfUrl;
      if( !empty($field['renditions']) ){
        $renditions = explode(",",$field['renditions']);
        ...
        if( $masterHeight == 0 && !empty($url) ){
          ...
          if(!empty($position)){
            ...
            $width_pos = strrpos(substr($url, 0, $height_pos), "_");
            $masterWidth = substr($url, ($width_pos+1), ($height_pos-$width_pos-1));
          }
        }
      ...
      ?>
        <ul class="acf-hl acf-soh-target">
            ...
            <li><a class="acf-icon -cancel dark" data-name="remove" href="#" data-renditions="<?php echo implode(",",$response); ?>"><i class="acf-sprite-delete"></i></a></li>
        </ul>
        <img data-name="image" id="the_cropped_img" src="<?php echo $url; ?>" alt=""/>
...
<?php
    ...
    ****/
    function get_image_data($field){
        ...
        $imageData->image_url = '';
        $imageData->renditions = $field['renditions'];
    ...
    */
    ...
    function perform_crop(){
        ...
        $date = new DateTime();
        $renditionsList = "";
        $masterString = $_POST['target_width']."x".$_POST['target_height'];
        if( !empty($_POST['renditions']) ){
          $renditions = explode(",",$_POST['renditions']);
          foreach( $renditions as $key => $rendition ){
            //$renditions[$key] = intval($rendition);
            $targetWidth = intval($rendition);
            $targetHeight = round(($targetWidth*$_POST['target_height'])/$_POST['target_width']);
            $imageData = $this->generate_cropped_image($_POST['id'], $_POST['x1'], $_POST['x2'], $_POST['y1'], $_POST['y2'], $targetWidth*2, $targetHeight*2, $saveToMediaLibrary, $_POST['preview_size'], "normal",$postid,$date);
            $imageData_half = $this->generate_cropped_image($_POST['id'], $_POST['x1'], $_POST['x2'], $_POST['y1'], $_POST['y2'], $targetWidth, $targetHeight, $saveToMediaLibrary, $_POST['preview_size'], "half",$postid,$date);
            $renditionsList .= $imageData['url'].",";
            $renditionsList .= $imageData_half['url'].",";
          }
        }
        $targetWidth = $_POST['target_width'];
        $targetHeight = $_POST['target_height'];
        $imageData = $this->generate_cropped_image($_POST['id'], $_POST['x1'], $_POST['x2'], $_POST['y1'], $_POST['y2'], $targetWidth, $targetHeight, $saveToMediaLibrary, $_POST['preview_size'], "normal",$postid,$date);
        $imageData_half = $this->generate_cropped_image($_POST['id'], $_POST['x1'], $_POST['x2'], $_POST['y1'], $_POST['y2'], round($targetWidth/2), round($targetHeight/2), $saveToMediaLibrary, $_POST['preview_size'], "half",$postid,$date);
        $renditionsList .= $imageData['url'].",";
        $renditionsList .= $imageData_half['url'];
        // $previewUrl = wp_get_attachment_image_src( $id, $_POST['preview_size']);
        // $fullUrl = wp_get_attachment_image_src( $id, 'full');
        $imageData['renditions'] = $renditionsList;
    ...
    function delete_crops(){
      ...
      $response['imgId'] = get_attachment_id( $_POST['img'] );
    ...
    function generate_cropped_image($id, $x1, $x2, $y1, $y2, $targetW, $targetH, $saveToMediaLibrary, $previewSize, $case, $postid,$date){//$id, $x1, $x2, $y$, $y2, $targetW, $targetH){
        ...
        // Set quality
        $image->set_quality( apply_filters('acf-image-crop/image-quality', 75) );
        ...
        if(! is_wp_error( $image ) ){
            ...
            if ( $case == 'half' ){
              ...
              $targetFilePath = $mediaDir['path'] . '/' . $retinaName;
            }
    ...
    */
    ...
    function format_value( $value, $post_id, $field ) {
       ...
       // validate
        ...
        {
            if(is_numeric($data->cropped_image)){
                $url = wp_get_attachment_url( $data->cropped_image );
                $attachmentData = wp_get_attachment_metadata($data->original_image);
                $width = $field['width'];
                if(!empty($field['height'])){
                  ...
                  $position = strpos($url, "_crop@2x");
                  $height_pos = strrpos(substr($url, 0, $position), "x");
                  $height = substr($url, ($height_pos+1), ($position-$height_pos-1));
                  $width_pos = strrpos(substr($url, 0, $height_pos), "_");
                  $width = substr($url, ($width_pos+1), ($height_pos-$width_pos-1));
                  $sizeString = $field['width']."x0";
                }
                $originalsizeString = $width."x".$height;
                $halfUrl = str_replace("@2x","",$url);
                $value = array();
                $value[$sizeString."_@2x"] = $url;
                $value[$sizeString] = $halfUrl;
                if( !empty($field['renditions']) ){
                  $renditions = explode(",",$field['renditions']);
                  foreach( $renditions as $rendition ) {
                    $rendition = intval($rendition);
                    $height = intval($height);
                    $width = intval($width);
                    if($height != 0){
                        $newHeight = round(($rendition*$height)/$width);
                    }else{
                      $newHeight = "";
                    }
                    $newString = $rendition."x".$newHeight;
                    if(!empty($field['height'])){
                      $newString_zero = $newString;
                    }else{
                      $newString_zero = $rendition."x0";
                    }
                    $newUrl = str_replace($originalsizeString,$newString,$url);
                    $value[$newString_zero."_@2x"] = $newUrl;
                    $halfUrl = str_replace("@2x","",$newUrl);
                    $value[$newString_zero] = $halfUrl;
                  }
                }
                //$value = $url;
            }
